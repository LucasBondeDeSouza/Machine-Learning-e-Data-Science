...
for _ in range(episodios):
    estado, _ = env.reset()  # Pegando apenas o estado
    penalidades, recompensa = 0, 0
    done = False
    
    while not done:
        acao = np.argmax(q_table[estado])  # Agora 'estado' é um número válido
        proximo_estado, recompensa, terminated, truncated, info = env.step(acao)
        done = terminated or truncated  # Ajuste para novas versões do Gym
        ...
        estado = proximo_estado  # Atualiza corretamente o estado
